...
1. Check that no discussion is currently open (error if one is).
2. Look up the record by ID in Notion (any type: DEC-NNN, TODO-NNN, RISK-NNN, PNT-NNN, QST-NNN, DISC-NNN).
...
9. Link the record:
   ...
   - Append a linked bullet to the appropriate section of the discussion page based on record type (RISK → Risks, QST-NNN → Open Questions, TODO → TODOs, PNT-NNN → Key Points, DEC-NNN → Decisions).
...
`[id]` may be any record ID: `DEC-NNN`, `DISC-NNN`, `TODO-NNN`, `RISK-NNN`, `PNT-NNN`, or `QST-NNN`.
